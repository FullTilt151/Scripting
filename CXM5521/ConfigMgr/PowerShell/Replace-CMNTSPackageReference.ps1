#region function Replace-CMNTSPackageReference
Function Replace-CMNTSPackageReference {
    <#
    .SYNOPSIS 

    .DESCRIPTION
        Replace all references to the specified package in any Task Sequence
    ...
    Param(
        ...
        [Parameter(Mandatory = $true, ValueFromPipelineByPropertyName = $true, ParameterSetName = 'TSByID')]
        [string[]]$TSPackageID,

        [Parameter(Mandatory = $true, ValueFromPipelineByPropertyName = $true, ParameterSetName = 'TSByName')]
        [string[]]$TSName,
    ...
    process {
        ...
        $References = foreach ($Instance in (Get-Variable -Name $VarToLoop -ValueOnly)) {
            $Query = switch ($PSCmdlet.ParameterSetName) {
                'ByName' {
                    [string]::Format("SELECT RefPackageID, ObjectName, PackageID FROM SMS_TaskSequencePackageReference_Flat WHERE ObjectName='{0}'", $Instance)
                }
                'ByID' {
                    [string]::Format("SELECT RefPackageID, ObjectName, PackageID FROM SMS_TaskSequencePackageReference_Flat WHERE ObjectID='{0}'", $Instance)
                }
            }
...
#endregion function Replace-CMNTSPackageReference