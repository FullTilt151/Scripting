...
If ($ContainerList -ne $null) {
        ForEach ($container in $ContainerList) {
            ...
            if ($fpath.ParentContainerNodeID -ne "0"){
               $ppath=$fpath
                Do{
                $ppath = (Get-WMIobject -Class SMS_ObjectContainerNode -Namespace root\sms\site_$($Site) -ComputerName $($Server) -Filter "ContainerNodeID='$($ppath.ParentContainerNodeID)'") 
                $buildpath = "$($ppath.Name)\$($fpath.Name)"
