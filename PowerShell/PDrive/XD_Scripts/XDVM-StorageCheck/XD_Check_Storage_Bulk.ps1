...
Connect-VIServer simvcswps05.rsc.humad.com
...
foreach ($row in $Machines)   #Loop for each VM
    ...
    $VMFreeSpace = Get-WmiObject Win32_logicaldisk -ComputerName $row.VM -filter "DeviceID='C:'"       #Grab available storage for C:
