...
Write-Host "++++VM Port and DDC Reset++++"
Write-Host "Would you like to reset the VDA Registration port and List of DDCs? (Y/N - No Default)"
...
if ($ResetRegPortAns -eq 'y' -or $ResetRegPortAns -eq 'Y')  
    {
    Write-Host "Would you like to reset the Registration Port and List of DDCs for:"
    Write-Host "1 - Unregistered VMs"
    Write-Host "2 - All VMs"
...
foreach ($row in $Machines)   #Loop for each VM
    ...
    Else 
        ...
        if ($ResetRegPort -eq $true)
            {
            Write-Host $row.VM ": Registration port and List of DDCs is being reset."
            $VMaddr=$null      #Clear the VM IP Address variable. 
            $RegPath= $null    #Clear the Registry Path variable. 
            ...
            &REG "Add" $RegPath "/v" "ListOfDDCs" "/t" "REG_SZ" "/d" "LOUXDCWPGX1S001.TS.HUMAD.COM LOUXDCWPGX1S002.TS.HUMAD.COM SIMXDCWPGX1S001.TS.HUMAD.COM SIMXDCWPGX1S002.TS.HUMAD.COM" "/f"
            if ($error.count -gt 0)   #Test if the above Registratio Port reset errored in any way.  Report the results. 
                {
                Write-Host $row.VM "Registration Port and List of DDCs reset failed"
