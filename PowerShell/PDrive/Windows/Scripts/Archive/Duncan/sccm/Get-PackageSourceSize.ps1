...
if(Test-Path(“C:\Program Files (x86)\Microsoft Configuration Manager\AdminConsole\bin\Microsoft.ConfigurationManagement.ApplicationManagement.dll”))
{
...
elseif(Test-Path(“C:\Program Files\Microsoft Configuration Manager\AdminConsole\bin\Microsoft.ConfigurationManagement.ApplicationManagement.dll”))
{
'Importing assemblies...'
#import assemblies
...
{
    throw [string]"Config Manager 2012 assemblies not found. Please run this script on machine that has Config Manager 2012 console installed."
}
...
gwmi -ComputerName $SiteServer -Namespace root\sms\site_$SiteCode -class sms_application | ?{$_.IsLatest -eq $true} | %{
 $i++
 Write-Progress -Activity "Gathering Application sources" -Status "Added: $i of $($ApplicationsQuery.count) " -PercentComplete (($i / $ApplicationsQuery.Count)*100)
...
Remove-Item "$Location\AppSources.csv"
...
{
    $d++
    Write-Progress -Activity "Calculating Application source sizes" -Status "Processed: $d of $($Sources.Count)" -PercentComplete (($d / $Sources.Count)*100)
...
{
    $i++
    Write-Progress -Activity "Gathering Package sources" -Status "Added: $i of $($PackagesQuery.count) " -PercentComplete (($i / $PackagesQuery.Count)*100)
...
{
    $d++
    Write-Progress -Activity "Calculating Package source sizes" -Status "Processed: $d of $($Sources.Count)" -PercentComplete (($d / $Sources.Count)*100)
...
{
    $i++
    Write-Progress -Activity "Gathering Image sources" -Status "Added: $i of $($ImagesQuery.count) " -PercentComplete (($i / $ImagesQuery.Count)*100)
...
Remove-Item "$Location\ImageSources.csv"
...
{
    $d++
    Write-Progress -Activity "Calculating Image source sizes" -Status "Processed: $d of $($Sources.Count)" -PercentComplete (($d / $Sources.Count)*100)
...
{
    $i++
    Write-Progress -Activity "Gathering Driver sources" -Status "Added: $i of $($DriversQuery.count) " -PercentComplete (($i / $DriversQuery.Count)*100)
...
Remove-Item "$Location\DriverSources.csv"
...
{
    $d++
    Write-Progress -Activity "Calculating Driver source sizes" -Status "Processed: $d of $($Sources.Count)" -PercentComplete (($d / $Sources.Count)*100)
...
'Results saved at "'+ $(Get-Location) + '\PackageSourceSizes.csv"'
