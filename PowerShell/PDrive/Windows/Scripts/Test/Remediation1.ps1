...
if (($regitem1 -ne $null) -or ($regitem2 -ne $null)) {Write-Output '32bit'} 
  else {$remediate =$true} 
  if (($regitem3 -ne $null) -or ($regitem4 -ne $null)) {Write-Output '64bit'} 
  else {$remediateSL =$true}
   
#Disable Adobe Update
if (($adobeB -ne 0) -or ($adobeA -ne 0)) {Write-Output 'Not Compliant'}
else {
$remediateadobe = $true
}
if(Test-Path "C:\Program Files (x86)\Mozilla Firefox" -IsValid) {Write-Output 'Firefox Path Exist'} 
else {$remediateAdobe = $true}

#Disable Java Update
Test-Path "HKLM:\SOFTWARE\WOW6432Node\JavaSoft\Java Update\Policy\EnableJavaUpdate"
if(Test-Path (("HKLM:\SOFTWARE\WOW6432Node\JavaSoft\Java Update\Policy\EnableJavaUpdate") -eq $false)) {Write-Output = $false}
else {$remediateJava = $true}
