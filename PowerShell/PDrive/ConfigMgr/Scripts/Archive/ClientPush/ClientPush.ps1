...
$comps = "WKPBD07DF","TRMJVVDH2"
#$comps = get-content \\wkmj029cw9\shared\scripts\sccm\ClientPush\computers.txt
    ...
    foreach ($comp in $comps) {
        if (Test-Connection $comp -count 1) {
            inlinescript {
                ...
                copy-item '\\wkmj029cw9\shared\sccm\SMS 2003 Toolkit 2\ccmclean.exe' -Destination "\\$comp\c$\temp"
                ...
                start-process -Wait -PSPath 'c:\PsExec.Exe' -ArgumentList "\\$comp c:\temp\ConfigMgr2012R2CU4\ccmsetup.exe /UsePKICert /NoCRLCheck SMSSITECODE=AUTO SMSCACHESIZE=52400 /mp:LOUAPPWPS611.rsc.humad.com /noservice /forceinstall"
            }
        } else {
