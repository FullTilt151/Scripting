    ...
    Process {
        ...
        for ($y = 2; $y -le $sheet.UsedRange.rows.Count; $y++) {
            ...
            if ($Collection) {
                ...
                If($colType -eq 'Auto'){[int]$startOffset = 15}
                else {[Int]$startOffset = 30}
                [Int]$endOffset = -60
                ...
                if ($logEntries) {
                    ...
                    New-CMNLogEntry -entry "Start day $($targetCollectionsStartDay[$CurrentCollection])" -type 1 @NewLogEntry
                    New-CMNLogEntry -entry "Start Time $($TargetCollectionsStartTime[$CurrentCollection])" -type 1 @NewLogEntry
                    New-CMNLogEntry -entry "Duration of $($targetCollectionsDuration[$CurrentCollection]) minutes" -type 1 @NewLogEntry
            ...
            if ($Server) {$TargetCollections[$CurrentCollection] += [Array]$Server}
        ...
        $excel.Quit()
        if ($logEntries) {New-CMNLogEntry -entry 'Finished with spreadsheet' -type 1 @NewLogEntry}
        ...
        if ($logEntries) {New-CMNLogEntry -entry 'Starting to cycle through collection list' -type 1 @NewLogEntry}
        ...
        foreach ($TargetCollection in $TargetCollections.GetEnumerator()) {
            ...
            if ($logEntries) {New-CMNLogEntry -entry "Working collection $($TargetCollection.Key)" -type 1 @NewLogEntry}
            ...
            if ($SCCMCollection) {
                if ($logEntries) {New-CMNLogEntry -entry "Collection $($SCCMCollection.Name) exists, removing rules." -type 1 @NewLogEntry}
                $SCCMCollection.Get()
                #Remove existing rules
                if ($SCCMCollection.CollectionRules) {
                    $SCCMCollection.DeleteMembershipRules($SCCMCollection.CollectionRules) | Out-Null
                }
                $CollectionIDsToKeep.add($SCCMCollection.CollectionID)
            }
            Else {
                if ($logEntries) {New-CMNLogEntry -entry "Creating collection $($TargetCollection.Key)." -type 1 @NewLogEntry}
            ...
            if ($targetCollectionsDuration[$targetCollection.Key] -ne 'None') {
                ...
                try {
                    if ($logEntries) {New-CMNLogEntry -entry "Attempting to remove any schedules from $($SCCMCollection.Name)" -type 1 @NewLogEntry}
                    ...
                    $SvcWindowItems = $CollectionSettings.GetArrayItems('ServiceWindows')
                    foreach ($SvcWindow in $SvcWindowItems) {
                        if ($logEntries) {New-CMNLogEntry -entry "Removing $($SvcWindow.Description)" -type 1 @NewLogEntry}
                        $SvcWindow.Remove
                    }
                    $CollectionSettings | Remove-WmiObject
                }
                Catch [System.Exception] {
                    if ($logEntries) {New-CMNLogEntry -entry "Error removing CollectionSettings for $($SCCMCollection.Name)" -type 3 @NewLogEntry}
                }
                if ($logEntries) {New-CMNLogEntry -entry "Creating $($targetCollection.Key) Collection Settings instance" -type 1 @NewLogEntry}
            ...
            if ($logEntries) {New-CMNLogEntry -entry "Removing items from $($SCCMCollection.Name)" -type 1 @NewLogEntry}
            $SCCMCollection.Get()
        ...
        if ($logEntries) {New-CMNLogEntry -entry 'Remove existing DeployTo Rules' -type 1 @NewLogEntry}
        foreach ($CollIdToUpdate in $CollIdsToUpdate) {
            ...
            if ($Collection.CollectionRules) {
                if ($logEntries) {New-CMNLogEntry -entry "Removing rules from $($Collection.Name) ($($Collection.CollectionID))" -type 1 @NewLogEntry}
                $Collection.DeleteMembershipRules($Collection.CollectionRules)
            }
        }

        if ($logEntries) {New-CMNLogEntry -entry 'Remove extra collections from Maintenance Window container' -type 1 @NewLogEntry}
        ...
        if ($logEntries) {New-CMNLogEntry -entry 'Building include rules for later' -type 1 @NewLogEntry}
        ...
        foreach ($mwCollectionID in $mwCollectionIDs) {
            ...
            if ($collection.Name -match 'NoRe' -or $collection.Name -match 'No Reboot') {
                if ($logEntries) {New-CMNLogEntry -entry "Adding $($collection.Name) ($($collection.CollectionID)) to noReboot rules" -type 1 @NewLogEntry}
                $noRebootColRules.Add($includeRule) | Out-Null
            }
            elseif ($collection.name -notmatch 'Do Not Patch') {
                if ($logEntries) {New-CMNLogEntry -entry "Adding $($collection.Name) ($($collection.CollectionID)) to reboot rules" -type 1 @NewLogEntry}
        ...
        if ($logEntries) {New-CMNLogEntry -entry 'Updating Deploy to collections with new include rules' -type 1 @NewLogEntry}
        foreach ($deployToCollectionID in $deployToCollectionIDs) {
            ...
            $collection = Get-WmiObject -Query $query @WMIQueryParameters
            if ($logEntries) {New-CMNLogEntry -entry "Processing $($collection.Name)" -type 1 @NewLogEntry}
            ...
            if ($collection) {
                ...
                if (-not($rebootCollection)) {
                    if ($logEntries) {New-CMNLogEntry -entry "$rebootCollectionName doesn't exist, creating." -type 1 @NewLogEntry}
                ...
                if (-not($noRebootCollection)) {
                    if ($logEntries) {New-CMNLogEntry -entry "$noRebootCollectionName doesn't exist, creating." -type 1 @NewLogEntry}
                ...
                if ($logEntries) {New-CMNLogEntry -entry "Adding rules to $rebootCollectionName" -type 1 @NewLogEntry}
                $rebootCollection.get()
                $rebootCollection.AddMemberShipRules($rebootColRules) | Out-Null

                if ($logEntries) {New-CMNLogEntry -entry "Adding rules to $noRebootCollectionName" -type 1 @NewLogEntry}
        ...
        if ($PSCmdlet.ShouldProcess($maintenanceWindowContainerID)) {
            ...
            foreach ($allProdGroups in $htServerGroups.GetEnumerator()) {
                ...
                foreach ($colID in $allProdGroups.value) {
                    ...
                    New-CMNDeviceCollectionIncludeRule -sccmConnectionInfo $sccmConnectionInfo -CollectionID $allProdCollection.CollectionID -includeCollectionID $colID -ruleName $colID -logFile $logFile -logEntries | Out-Null
                }
            }
            ...
            foreach ($allManualGroups in $htManualGroups.GetEnumerator()) {
                ...
                foreach ($colID in $allManualGroups.value) {
                    New-CMNLogEntry -entry "Adding include rule for collection $colID" -type 1 @NewLogEntry
                    New-CMNDeviceCollectionIncludeRule -sccmConnectionInfo $sccmConnectionInfo -CollectionID $allManualCollection.CollectionID -includeCollectionID $colID -ruleName $colID -logFile $logFile -logEntries | Out-Null
                }
            }
...
Import-CMNMainteananceWindowCollections -SCCMConnectionInfo $SCCMCon -file $file -limitingCollectionID $lmtgCollectionID -RebootPackageID $RebootPackageID -DestinationMWContainer $DestinationMWContainer -DeploymentFolder $DeploymentFolder -collectionColumn 1 -nameColumn 2 -logFile $LogFile -logEntries -showProgress 
...
New-CMNLogEntry -entry "Getting container ID for $DeploymentFolder" -type 1 -logFile $LogFile -component 'import-CMNMainteananceWindowCollections'
$destinationMWContainerID = Get-CMNObjectContainerNodeIDbyName -sccmConnectionInfo $SCCMCon -Name $DestinationMWContainer -ObjectType SMS_Collection_Device -logFile $logFile -logEntries
Import-CMNMainteananceWindowCollections -SCCMConnectionInfo $SCCMCon -file $file -limitingCollectionID $lmtgCollectionID -RebootPackageID $RebootPackageID -DestinationMWContainer $DestinationMWContainer -DeploymentFolder $DeploymentFolder -collectionColumn 1 -nameColumn 2 -logFile $LogFile -logEntries -showProgress
Set-CMNServerDayCollections -sccmConnectionInfo $SCCMCon -limitingCollectionID $lmtgCollectionID -maintenanceWindowContainerID $destinationMWContainerID -logFile $logFile -logEntries
