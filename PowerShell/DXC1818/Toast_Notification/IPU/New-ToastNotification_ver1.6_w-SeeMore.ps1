...
try {
	...
	$More = $Xml.Configuration.Option | Where-Object {$_.Name -like 'More'} | Select-Object -ExpandProperty 'Value'
	...
	$MoreButtonEnabled = $Xml.Configuration.Option | Where-Object {$_.Name -like 'MoreButton'} | Select-Object -ExpandProperty 'Enabled'
    $MoreButtonContent = $Xml.Configuration.Option | Where-Object {$_.Name -like 'MoreButton'} | Select-Object -ExpandProperty 'Value'
...
# Create the default toast notification XML with action button and dismiss button and more button
if (($ActionButtonEnabled -eq "True") -AND ($DismissButtonEnabled -eq "True") -AND ($MoreButtonEnabled -eq "True")) {
    Write-Log -Message "Creating the xml for displaying both action button and dismiss button and more button"
[xml]$Toast = @"
<toast scenario="$Scenario">
    ...
    <actions>
		...
		<action activationType="protocol" arguments="$More" content="$MoreButtonContent" />
...
# NO action button and NO dismiss button and NO more button
if (($ActionButtonEnabled -ne "True") -AND ($DismissButtonEnabled -ne "True") -AND ($MoreButtonEnabled -ne "True")) {
    Write-Log -Message "Creating the xml for no action button and no dismiss button and no more button"
[xml]$Toast = @"
<toast scenario="$Scenario">
    <visual>
    <binding template="ToastGeneric">
...
# Action button, More button and NO dismiss button
if (($ActionButtonEnabled -eq "True") -AND ($MoreButtonEnabled -eq "True") -AND ($DismissButtonEnabled -ne "True")) {
...
# Dismiss button and NO action button and NO more button
if (($ActionButtonEnabled -ne "True") -AND ($MoreButtonEnabled -ne "True") -AND ($DismissButtonEnabled -eq "True")) {
