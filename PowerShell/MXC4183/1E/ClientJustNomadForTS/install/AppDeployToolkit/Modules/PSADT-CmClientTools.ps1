    ...
    process {
        if ($os.ProductType -eq 1) {
            ...
            elseif ($env:COMPUTERNAME -match '...PXE.*') {
                Write-Log -Message 'This is an OSD Master, setting CCMCache to 409600 (400GB) and ccmCacheDir to F:\CCMCache and LocalCachePath to F:\NomadCache'
                $isOSDmaster = $true
                $ccmCacheSize = 409600
                $ccmCacheDir = 'F:\'
                $doInstallNomad = $true
            ...
            if ($computerSystem.Domain -eq 'ts.humad.com' -or $computerSystem.Domain -eq 'tmt.loutms.tree') {
                Write-Log -Message 'Machine in Citrix domain, checking if persistent or non-persistent'
                if ((($computerSystem.Name -notmatch '...[CX][MA][FH].*' -or $computerSystem.Name -match '...[CX][MA][ABCDFLNSPX].....S.*')) -and $computerSystem.Name -notmatch '......WP[VU].*') {
                    Write-Log -Message 'This is a Citrix persistent server, setting ccmCacheDir to D:\Program Files\CCMCache and logs to D:\Program Files\CCM\Logs'
