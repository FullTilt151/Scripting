param (
	...
	$Config
	# name of the config xml file stored in the same directory as the script
)
...
#>
	...
	#region load XML and settings
	if (!$PSBoundParameters.ContainsKey('Config')) {
		$Config = "$(Split-Path -Path $PSCommandPath)\$((Split-Path -Path $PSCommandPath -Leaf) -Replace '.ps1', $null -replace '.exe', $null).xml"
		if (!(Test-Path $Config)) {
			[void][System.Windows.Forms.MessageBox]::Show("Unable to find a config file!`nPlease use the config parameter or have a .xml with the same name as the script in the script directory", "", 'OK', 'Error')
			Break
		}
	}
	[xml]$Settings = Get-Content -Path $Config
	$script:Notifications = [System.Convert]::ToBoolean($Settings.Settings.EmailSettings.NotificationsEnabled)
	$script:FromEmail = $Settings.Settings.EmailSettings.FromAddress
	$script:SMTPServer = $Settings.Settings.EmailSettings.SMTPServer
	$script:PrimarySiteServer = $Settings.Settings.GeneralSettings.PrimarySiteServer
	$script:DPTableType = $Settings.Settings.DistributionSettings.DPTableType
	$script:Action = $Settings.Settings.DistributionSettings.Action
	$script:DistributionCheckInterval = $Settings.Settings.DistributionSettings.DistributionCheckInterval
	$script:LogPath = $Settings.Settings.GeneralSettings.LogPath
	#endregion load XML and settings
	...
	$script:MachineCIMSession = New-CimSession -Name "MachineCIMSession"
	$script:SiteCode = (Get-CimInstance -CimSession $script:ServerCIMSession -Namespace "root\sms" -ClassName "__Namespace").Name.Substring(5, 3)
...
#region Source: Globals.ps1
	...
	#region defaults
	$script:DefaultTimezone = "(UTC-05:00) Eastern Time (US & Canada)" #must be in the "Display Name" format so it matches an item on the form
	#endregion defaults
			...
			if ((($file).length) -ige $filesize) #this starts the log roll 
	{
				...
				$files = Get-ChildItem $filedir | Where-Object{
					$_.name -like "$fn*"
				...
				for ($i = ($files.count); $i -gt 0; $i--) {
					...
					$operatingFile = $files | Where-Object{
						($_.name).trim($fn) -eq $i
					}
					if ($operatingfile) {
						$operatingFilenumber = ($files | Where-Object{
								($_.name).trim($fn) -eq $i
							}).name.trim($fn)
					}
					else {
					...
					elseif ($i -ge $logcount) {
						if ($operatingFilenumber -eq $null) {
							...
							$operatingFile = $files | Where-Object{
								($_.name).trim($fn) -eq $operatingFilenumber
							}
	...
	function Get-TotalPackageSize {
		$TotalSizeInKB = 0
		$SelectedPackages = $script:ContentTable | Where-Object {
			...
			if ($script:DependencyCheck -ne "Started") {
				...
				Add-JobTracker -Name "ContentDependencyCheck" -JobScript {
					...
					foreach ($Item in $script:ContentTable) {
						if ($Item.Type -eq "8") {
							$CIID = ($script:CIIDs | Where-Object {
									$_.ModelName -eq $Item.SecurityKey
								}).CI_ID
							$Item.CIID = $CIID
							$Item.Dependencies = ($script:AppDependenceRelationships | Where-Object {
									$_.FromApplicationCIID -eq $CIID
								}).ToApplicationCIID
						}
					}
	...
	function Get-CMDistributionPoints {
		...
		foreach ($DP in $DPs) {
			$ReturnValue.Add([pscustomobject] @{
					Name	    = $($DP.Name);
	...
	function Get-CMTaskSequenceReferencedContent ($TSPackageID) {
		$TSRef = ($script:TaskSequenceReferences | Where-Object {
				$_.PackageID -eq $TSPackageID
			}).RefPackageID
		foreach ($Item in $script:ContentTable) {
	...
	function Get-CMDependencyChains {
		...
		do {
			$Dependencies = ($script:ContentTable | Where-Object {
					$_.ID -eq $PackageID
				}).Dependencies
			foreach ($D in $Dependencies) {
				$CIID = $D
				$PackageID = ($script:ContentTable | Where-Object {
						$_.CIID -eq $D
					}).Id
	...
	function Start-ContentDistribution {
		...
		Write-CMLogEntry -Value "Start-ContentRedistribution [PackageIDs=$($PackageIDs -join ';')] [DPArray=$($DPArray -join ';')] [SiteCode=$SiteCode] [PrimarySiteServer=$PrimarySiteServer] [DPTableType=$DPTableType] [Namespace=$Namespace]" -Component "ScheduledJob"
		switch ($DPTableType) {
			DP {
				foreach ($DP in $DPArray) {
					foreach ($Package in $PackageIDs) {
						if (!($CIMObject = Get-CimInstance -CimSession $CimSession -Namespace "$Namespace" -ClassName SMS_DistributionPoint -Filter "ServerNALPath LIKE '%$($DP.Name)%' and PackageID='$Package'")) {
							try {
								$Properties = @{
									PackageID = "$Package"; ServerNALPath = "$($DP.NALPath)"; SiteCode = "$SiteCode"
								}
			...
			DP {
				foreach ($DP in $DPArray) {
					foreach ($Package in $PackageIDs) {
						if ($CIMObject = Get-CimInstance -CimSession $CimSession -Namespace "$Namespace" -ClassName SMS_DistributionPoint -Filter "ServerNALPath LIKE '%$($DP.Name)%' and PackageID='$Package'") {
							try {
								$CIMObject | Set-CimInstance -CimSession $CimSession -Property @{
									RefreshNow = $true
								}
	...
	function Start-ContentUpdate {
		...
		foreach ($Package in $Packages) {
			$PackageType = @{
				...
				259 = 'SMS_OperatingSystemInstallPackage'
			}
...
"@ -IgnoreWarnings | Out-Null
	...
	$formContentDistribution_Load = {
		if (!$script:Notifications) {
			$textbox_NotificationRecipient.Text = 'Disabled'
			$textbox_NotificationRecipient.Enabled = $false
			$errorprovider.SetError($textbox_NotificationRecipient, $null)
		}
		else {
			$textbox_NotificationRecipient.Text = $null
			$textbox_NotificationRecipient.Enabled = $true
		...
		$paramAddJobTracker = @{
			...
			CompletedScript = {
				Receive-Job -Name "SMS_PackageBaseclass" -OutVariable script:Results
			}
		...
		$paramAddJobTracker = @{
			...
			CompletedScript = {
				Receive-Job -Name "SMS_ApplicationLatest" -OutVariable script:CIIDs
			}
			ArgumentList = $Namespace, $PrimarySiteServer, $script:Credentials
		}
		Add-JobTracker @paramAddJobTracker
		$paramAddJobTracker = @{
			...
			CompletedScript = {
				Receive-Job -Name "SMS_TaskSequencePackageReference_Flat" -OutVariable script:TaskSequenceReferences
			}
			ArgumentList = $Namespace, $PrimarySiteServer, $script:Credentials
		}
		Add-JobTracker @paramAddJobTracker
		$paramAddJobTracker = @{
			...
			CompletedScript = {
				Receive-Job -Name "SMS_AppDependenceRelation" -OutVariable script:AppDependenceRelationships
			}
			ArgumentList = $Namespace, $PrimarySiteServer, $script:Credentials
		}
		Add-JobTracker @paramAddJobTracker
...
"@ | Out-Null
	...
	$textbox_AvailableDPGroupsFilter_TextChanged = {
		...
		if ($Filter -ne "" -and $Filter -ne $null) {
			foreach ($Group in $script:DPTable) {
				if ($Group.Name -notmatch $Filter -and $Group.Description -notmatch $Filter -and $null -ne $Group.IsVisible) {
					$Group.IsVisible = $false
				}
				else {
					if ($null -ne $Group.IsVisible) {
	...
	$listview_AvailableDPGroups_MouseDoubleClick = [System.Windows.Forms.MouseEventHandler]{
		...
		if ($hit.Item) {
			foreach ($Group in $script:DPTable) {
				if ($Group.Name -eq $hit.Item.Text -and $null -ne $Group.IsSelected) {
					$Group.IsSelected = $true
				}
			}
		}
	...
	$combobox_AvailableContentType_SelectedIndexChanged = {
		...
		foreach ($Item in $script:ContentTable) {
			...
			if ($Item.Type -eq $script:Type -and $null -ne $Item.IsVisible) {
				$Item.IsVisible = $true
			}
			else {
				if ($null -ne $Item.IsVisible) {
	...
	$textbox_AvailableContentFilter_TextChanged = {
		...
		if ($Filter -ne "" -and $Filter -ne $null) {
			foreach ($Package in ($script:ContentTable | Where-Object {
						$_.Type -eq $script:Type
					})) {
				if ($Package.Name -notmatch $Filter -and $Package.Description -notmatch $Filter -and $null -ne $Package.IsVisible) {
					$Package.IsVisible = $false
				}
				else {
					if ($null -ne $Package.IsVisible) {
	...
	$button_ResetSelectedContent_Click = {
		...
		foreach ($Package in $script:ContentTable) {
			if ($Package.Type -eq $script:Type -and $null -ne $Package.IsVisible) {
				$Package.IsVisible = $true
			}
			else {
				if ($null -ne $Package.IsVisible) {
	...
	$tabpage_Summary_Enter = {
		...
		if ($combobox_AvailableContentType.SelectedItem.ToString() -ne "TaskSequence") {
			...
			$TSPackageID = ($script:ContentTable | Where-Object {
					$_.IsSelected -eq $true
				}).ID
	...
	$buttonSchedule_Click = {
		...
		$SelectedDP = $script:DPTable | Where-Object {
			$_.IsSelected -eq $true
		}
		$SelectedPackages = $script:ContentTable | Where-Object {
			$_.IsSelected -eq $true
		...
		try {
			...
			if ($Script:StartTime -lt (get-date)) {
				$Script:StartTime = $Script:StartTime.AddMinutes(5)
			}
			$paramRegisterScheduledJob = @{
				Name		 = $TaskName
				ArgumentList = $script:Notifications, $Recipients, $FromEmail, $SMTPServer, $script:DPTableType, $SelectedDP, $SelectedPackages, $TotalContentSize, $script:Action, $Script:DistributionCheckInterval, $script:LogPath, $PrimarySiteServer, $SiteCode, $allFunctionDefs
				ScriptBlock  = {
					param ($Notifications,
						$Recipients,
						$FromEmail,
					...
					New-LoopAction -LoopTimeout 4 -LoopTimeoutType Minutes -LoopDelay 10 -ExitCondition {
						($script:InProgress -and $script:Started)
					} -ScriptBlock {
						$script:InProgress = Get-CimInstance -CimSession $script:ServerCIMSession -Namespace $Namespace -ClassName SMS_DistributionStatus -Filter "Type='2' or Type='4'" | Where-Object {
							$_.PackageID -in ($script:SelectedPackages | Select-Object -ExpandProperty ID)
						}
						if ($script:InProgress) {
							$DistribStartBody = "<div>Scheduled ""$script:Action"" has started. Below is some information regarding the distribution.</div>
									<div>&nbsp;</div>
									<ul>
									<li><strong>Package(s):</strong> $script:SelectedPackageNames</li>
									<li><strong>PackageID(s):</strong> $script:SelectedPackageIDs</li>
									<li><strong>Content size:</strong> $script:TotalContentSize</li>
									<li><strong>$script:DPTableType(s):</strong> $script:SelectedDPNames</li>
									<li><strong>Start Time:</strong> $script:DistStart</li>
									</ul>"
							if ($script:Notifications) {
								Send-MailMessage -From $script:FromEmail -To $script:Recipients -SmtpServer $script:SMTPServer -Subject "Scheduled $script:Action Started" -Body $DistribStartBody -BodyAsHtml
							}
							$script:Started = $true
						}
					} -IfTimeoutScript {
					...
					New-LoopAction -LoopTimeout 48 -LoopTimeoutType Hours -LoopDelay $DistributionCheckInterval -ExitCondition {
						($script:InProgress.Count -eq 0 -and $script:Started)
					} -ScriptBlock {
						$script:InProgress = Get-CimInstance -CimSession $script:ServerCIMSession -Namespace $Namespace -ClassName SMS_DistributionStatus -Filter "Type='2' or Type='4'" | Where-Object {
							$_.PackageID -in ($script:SelectedPackages | Select-Object -ExpandProperty ID)
						}
						...
						$Failed = $script:InProgress | Where-Object {
							$_.Type -eq '4'
