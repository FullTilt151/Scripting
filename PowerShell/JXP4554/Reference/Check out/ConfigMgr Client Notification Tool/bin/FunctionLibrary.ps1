###################################################################
##                                                               ##
## Contains functions used by ConfigMgr Client Notification Tool ##
##                                                               ##
###################################################################

# Function to get the client online status from WMI on the site server
Function Get-ClientOnlineStatus {
    Param($UI,$SiteServer,$Resources)

    # Create a datatable to hold the results
    $DataTable = New-Object System.Data.DataTable
    [void]$DataTable.Columns.Add([System.Data.DataColumn]::new("Online"))
    [void]$DataTable.Columns.Add([System.Data.DataColumn]::new("ResourceID"))
    [void]$DataTable.Columns.Add([System.Data.DataColumn]::new("ComputerName"))

    # Populate with initial data
    Foreach ($Resource in $Resources)
    {
        [void]$DataTable.Rows.Add("$($UI.DataSource[4])\bin\icon11.bmp","","$resource")
    }
    $UI.DataSource[3] = $DataTable

    # Try a Cimsession to the site server
    Try
    {
    ...
    Catch
    {
        $UI.DataSource[1] = "Could not create a CimSession to $SiteServer`: $_"
        $UI.DataSource[2] = "Red"
    ...
    # Get SiteCode
    $SiteCode = Get-CimInstance -CimSession $CimSession -Namespace ROOT\SMS -Query "Select SiteCode from SMS_ProviderLocation where ProviderForLocalSite='True'" | Select -ExpandProperty SiteCode

    # Get the data from the SMS provider and update the icons
    Foreach ($Resource in $Resources)
    {
        $UI.DataSource[1] = "Retrieving online status for $Resource"
        $Query = "SELECT SMS_CN_ClientStatus.OnlineStatus, SMS_R_System.ResourceID FROM SMS_R_System inner join SMS_CN_ClientStatus on SMS_R_System.ResourceID = SMS_CN_ClientStatus.ResourceID WHERE SMS_R_System.Name='$Resource'"
        $Result = Get-CimInstance -CimSession $CimSession -Namespace ROOT\SMS\Site_$SiteCode -Query $Query
        $OnlineStatus = $Result.SMS_CN_ClientStatus.OnlineStatus
        $ResourceID = $Result.SMS_R_System.ResourceId
        $Row = $UI.DataSource[3].Select("ComputerName='$resource'")
        If ($Row)
        {
            If ($OnlineStatus -eq 1)
            {
                $Row[0]["Online"] = "$($UI.DataSource[4])\bin\icon8.bmp"
            }
            If ($OnlineStatus -eq 0)
            {
                $Row[0]["Online"] = "$($UI.DataSource[4])\bin\icon10.bmp"
            }
            If ($ResourceID)
            {
                $Row[0]["ResourceID"] = $ResourceID
            }            
        }
    }
...
# Function to trigger client notification actions
Function Trigger-ClientNotification {
    Param($UI,$SiteServer,$Type,$Resources)

    # Try a PSsession to site server
    Try
    {
        $UI.DataSource[1] = "Creating a PSSession to $SiteServer"
        $UI.DataSource[2] = "Black"
        $PSSession = New-PSSession -ComputerName $SiteServer -ErrorAction Stop
    }
    Catch
    {
        $UI.DataSource[1] = "Could not create a PSSession to $SiteServer`: $_"
        $UI.DataSource[2] = "Red"
        $UI.DataSource[0] = "False"
        Return
    }
    ...
    # Set the Primary action type based on selected Type
    Switch($Type)
    {
        "Download Computer Policy" { $PrimaryActionType = 8 }
        "Download User Policy" { $PrimaryActionType = 9 }
        "Collect Discovery Data" { $PrimaryActionType = 10 }
        "Collect Software Inventory" { $PrimaryActionType = 11 }
        "Collect Hardware Inventory" { $PrimaryActionType = 12 }
        "Evaluate Application Deployments" { $PrimaryActionType = 13 }
        "Evaluate Software Update Deployments" { $PrimaryActionType = 14 }
        "Switch to next Software Update Point" { $PrimaryActionType = 15 }
        "Evaluate Device Health Attestation" { $PrimaryActionType = 16 }
        "Check Conditional Access Compliance" { $PrimaryActionType = 125 }
        "Restart" { $PrimaryActionType = 17 }
    }
  
    # Invoke the code to perform the client operations
    $UI.DataSource[1] = "Invoking client operations against online resources"
    $UI.DataSource[2] = "Black"

    $Code = {
        Param([array]$ResourceIDs,$PrimaryActionType)
        ...
        $Dictionary.Add("Type", $PrimaryActionType)
        $Dictionary.Add("TargetCollectionID", "SMS00001")
        $Dictionary.Add("TargetResourceIDs", $ResourceIDs)
        $Result = $WQLConnectionManager.ExecuteMethod("SMS_ClientOperation","InitiateClientOperation",$Dictionary)
        Return $Result.PropertyList.ReturnValue              
    }

    Try
    {
        $Command = Invoke-Command -Session $PSSession -ScriptBlock $Code -ArgumentList $Resources,$PrimaryActionType -ErrorAction Stop
    }
    Catch
    {
        $UI.DataSource[1] = "Could not trigger client notification action: $_"
        $UI.DataSource[2] = "Red"
        $UI.DataSource[0] = "False"
