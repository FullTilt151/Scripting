Function Foreach-CMNParallel {
    ...
    Param(
        [Parameter(Mandatory = $true, HelpMessage = 'This is where you provide the PowerShell Cmdlet / Script file that you want to multithread.')]
        [System.Management.Automation.ScriptBlock]$Command,

        [Parameter(Mandatory = $false, ValueFromPipeline = $true, ValueFromPipelineByPropertyName = $true, HelpMessage = 'The objectlist represents the arguments that are provided to the child script.')]
        $ObjectList,

        [Parameter(Mandatory = $false, HelpMessage = 'This allows you to specify the parameter for which your input objects are to be evaluated.')]
        $InputParam = $Null,

        [Parameter(Mandatoy = $false, HelpMesage = 'This allows you to specify additional parameters to the running command.')]
        [HashTable]$AddParam = @{},

        [Parameter(Mandatory = $false, HelpMessage = 'This allows you to add additional switches to the command you are running.')]
        [Array]$AddSwitch = @(),

        [Parameter(Mandatory = $false, HelpMessage = 'This is the maximum number of threads to run at any given time. The default value is 20.')]
        [Int]$MaxThreads = 20,

        [Parameter(Mandatory = $false, HelpMessage = 'This is the time between cycles of the child process detection cycle. The default value is 200ms.')]
        $SleepTimer = 200,

        [Parameter(Mandatory = $false, HelpMessage = 'This is the maximum amount of time the script is allowed to run. The default is 120 seconds')]
        $MaxResultTime = 120
    )
