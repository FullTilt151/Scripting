...
$extensionGUID = 'mpioohaaamocbdblijfoeigkkadcekli;https://edge.microsoft.com/extensionwebstorebase/v1/crx'
...
$fetchUrl = (Get-ItemProperty -Path 'HKLM:\Software\Policies\Microsoft\Edge\3rdparty\extensions\mpioohaaamocbdblijfoeigkkadcekli\policy' -name "fetchUrl").fetchurl

$AdminModeValue = (Get-ItemProperty -Path 'HKLM:\Software\Policies\Microsoft\Edge\3rdparty\extensions\mpioohaaamocbdblijfoeigkkadcekli\policy' -name "adminmode").adminmode
...
if ($remediate -eq $true -and $compliant -eq $false) {
    ...
    $PolicyPath = 'HKLM:\Software\Policies\Microsoft\Edge\3rdparty\extensions\mpioohaaamocbdblijfoeigkkadcekli\policy'
    if((Test-Path $PolicyPath) -ne $true) {
        New-Item -Path $PolicyPath -Force
    }

    if ($FetchUrl -ne $FetchUrlValue) {
        New-ItemProperty -Path 'HKLM:\Software\Policies\Microsoft\Edge\3rdparty\extensions\mpioohaaamocbdblijfoeigkkadcekli\policy' -Name 'fetchUrl' -Value $FetchUrlValue -Force
    }

    if ($AdminModeValue -ne 0) {
        New-ItemProperty -Path 'HKLM:\Software\Policies\Microsoft\Edge\3rdparty\extensions\mpioohaaamocbdblijfoeigkkadcekli\policy' -Name 'adminmode' -Value 0 -Force
    }
} else {
    $compliant
}