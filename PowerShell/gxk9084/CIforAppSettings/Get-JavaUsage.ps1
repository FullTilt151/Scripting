$LogFile = "$env:SystemDrive\temp\CM_JavaUsageLogging.log"
$LoggingEnable = $True
    ...
    process {
        try {
            ...
            IF ($LoggingEnable -eq $true) {
                Log-ScriptEvent -Value "CM_JavaUsageTracking class creation complete." -Severity 1
            }
        }
        CATCH {
            IF ($LoggingEnable -eq $true) {
                Log-ScriptEvent -Value "CM_JavaUsageTracking class creation error." -Severity 3
            }
        }
    }
}
...
IF ($LoggingEnable -eq $true) {
    Log-ScriptEvent -Value "Starting Java logging discovery." -Severity 1
}
...
ForEach ($JRE in $JREs) {
    IF ($LoggingEnable -eq $true) {
        Log-ScriptEvent -Value "Interrogating JRE path $($JRE.JavaHome)" -Severity 1
    }
    ...
    if ($JREPath) {
        ...
        if (-Not $UTProps -or $OverwriteUT) {
            IF ($LoggingEnable -eq $true) {
                Log-ScriptEvent -Value "Creating $($JRE.JavaHome)\lib\management\usagetracker.properties" -Severity 1
            }
            Create-UsageTrackingProps -UTPath "$($JRE.JavaHome)\lib\management\usagetracker.properties"
        }
        else {
            IF ($LoggingEnable -eq $true) {
                Log-ScriptEvent -Value "$($JRE.JavaHome)\lib\management\usagetracker.properties exists" -Severity 1
            }
        }
    }
}
...
try {
    IF ($LoggingEnable -eq $true) {
        Log-ScriptEvent -Value "Gather user profile paths." -Severity 1
    }
...
Foreach ($user in $users) {
    ...
    IF ($LoggingEnable -eq $true) {
        Log-ScriptEvent -Value "Checking for $($user.LocalPath)\$($UTLogFileName)" -Severity 1
    }
    $path = Test-Path "$($user.LocalPath)\$($UTLogFileName)"
    
    if ($path) {
        IF ($LoggingEnable -eq $true) {
            Log-ScriptEvent -Value "Found $($user.LocalPath)\$($UTLogFileName), attempting to load data." -Severity 1
        }
        Try {
            ...
            $Dataset += $Data | Select-Object @{Name = "User"; Expression = { ($($user.LocalPath) -split '\\')[-1].ToString() } }, Type, DateTime, HostIP, @{Name = "Command"; Expression = { if ($_.Command -like 'http*') {
                        ($_.Command -split ': ')[0].ToString() 
                    }
                    else {
                        ($_.Command -split ':')[0]
                    } }
            ...
            IF ($LoggingEnable -eq $true) {
                Log-ScriptEvent -Value "Parsing $($user.LocalPath)\$($UTLogFileName)" -Severity 1
            }
        }
        Catch {
            Write-Host "Wowzzers" #Wicked error here
            IF ($LoggingEnable -eq $true) {
                Log-ScriptEvent -Value "Error parsing $($user.LocalPath)\$($UTLogFileName)" -Severity 3
            }
            Exit 5150
        }
    }
...
IF ($LoggingEnable -eq $true) {
    Log-ScriptEvent -Value "Completed data discovery, writing data to WMI" -Severity 1
}
...
try {
    ForEach ($Record in $DataSet) {
        if ($Record.Command.Trim()) {
            $Instance = Get-WmiObject -Query "select * from CM_JavaUsageTracking where DateTime='$($Record.DateTime)'"
