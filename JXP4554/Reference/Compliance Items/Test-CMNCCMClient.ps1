...
if($lastMessageDelta.Hours -gt 24){$stateMessage = $false}
else{$stateMessage = $true}
...
$lastMessageDelta = New-TimeSpan -Start $(Get-Date) -End $lastEvalTime
if($lastMessageDelta.Hours -gt 24){$ccmEval = $false}
else{$ccmEval = $true}
...
#CCMEval Log
$EvalLog = (Get-Content C:\windows\ccm\logs\CcmEval.log | select-string 'Successfully evaluated all client health rules.' | Select-Object -Last 1).tostring()
$datestart = $EvalLog.IndexOf('date=')
$date = (($EvalLog.Substring($datestart)).Split(' ')[0]).Replace('date="','').Replace('"','')
$ccmEvalLog = (New-TimeSpan -Start $date -End (get-date)).Days -eq 0

Write-Output ($stateMessage -and $ccmEval -and $ccmEvalLog -and $services)