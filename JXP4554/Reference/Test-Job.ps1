...
Function Invoke-Loop
{
	...
	New-LogEntry "Working on Package $($Package.PackageID) - $($Package.Name)"
	sleep 60
}
    ...
    Switch ($PackageID.PackageType)
    {
        0
        {
            ...
            $ArgList = @{
                ...
                PackageType = $($objPackage.PackageType)
                IsDPRemoved = $false
                LogLevel = $LogLevel
                LogFile = $LogFile
            }
			While($(Get-Job | Where-Object {$_.JobStateInfo.State -ne 'Completed'} | Measure-Object).Count -ge 10)
			{Sleep 10}
            Start-Job -FilePath $($ScriptDir)\invoke-Loop.ps1 -Name "Job $($Package.PackageID)" -ArgumentList $ArgList
        }
    }
}
