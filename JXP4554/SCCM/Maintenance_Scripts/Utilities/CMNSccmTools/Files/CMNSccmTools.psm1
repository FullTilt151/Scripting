...
<# workflow delete-cmnDeclinedUpdates {
    [CmdletBinding(ConfirmImpact = 'Low')]
    param (
        [Parameter($Mandatory = $true)]
        [String]$wsusServer,
    ...
    New-CMNLogEntry -entry "Starting WSUS Cleanup section" -type 1 -logFile $logFile -component 'delete-cmnDelcinedUpdates'
    [reflection.assembly]::LoadWithPartialName("Microsoft.UpdateServices.Administration") | out-null 
    New-CMNLogEntry -entry "Loaded assembly, connecting to $($myFQDN):$port - SSL $useSSL" -type 1 -logFile $logFile -component 'delete-cmnDelcinedUpdates'
    $wsus = [Microsoft.UpdateServices.Administration.AdminProxy]::GetUpdateServer($myFQDN, $useSSL, $port); 
    New-CMNLogEntry -entry "Connected to WSUS" -type 1 -logFile $logFile -component 'delete-cmnDelcinedUpdates'    
    ForEach -Parallel ($declineGuid in $declineGuids){
        Write-Verbose "Deleting $declineGuid"
        $wsus.DeleteUpdate($declineGuid)
    }
} #>
...
Function Get-CMNSCCMConnectionInfo {
    ...
    process {
        ...
        if (-not($SiteCode)) {
            if ($PSBoundParameters['logEntries']) {New-CMNLogEntry -entry "Unable to connect to Site Server $SiteServer" -type 3 @NewLogEntry}
...
Function New-CMNLogEntry {
    ...
    if ($tzOffset -lt 0) {$tzOffset = "$(Get-Date -Format "HH:mm:ss.fff")+$(-$tzOffset)"}
    #otherwise, we need to represent it as -###
    else {$tzOffset = "$(Get-Date -Format "HH:mm:ss.fff")-$tzOffset"}
...
Export-ModuleMember -Function delete-cmnDeclinedUpdates
