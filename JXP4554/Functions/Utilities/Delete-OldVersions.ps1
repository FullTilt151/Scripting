Function Delete-OldVersions  ($FilePath, $LogFile)
{
    $Files = Get-ChildItem -Path $FilePath | Where-Object {! $_.PSIsContainer} | Sort-Object
    for($x = 0;$x -lt $Files.count;$x++)
    {
        for($y = 0;$y -lt $Files.count - 1;$y++)
        {
            if ($y -ne $x)
            {
                ...
                if ($i -eq $j)
                {
                    [int]$a = $Files[$x].Name -replace '.*\.(.*)\.tar', '$1'
                    [int]$b = $Files[$y].Name -replace '.*\.(.*)\.tar', '$1'
                    if ($a -gt $b)
                    {
                        if (Test-Path $Files[$y].FullName)
                        {
                            New-LogEntry "Deleting $($Files[$y].FullName)" 1 $ScriptName $LogFile
                    ...
                    else
                    {
                        ...
                        {
                            New-LogEntry "Deleting $($Files[$x].FullName)" 1 $ScriptName $LogFile
...
Function New-LogEntry ($Entry, $Type, $Component, $FilePath) {
    ...
    $Entry | Out-File -FilePath $FilePath -Append -Encoding ascii
}
...
New-LogEntry "Starting - $LogFile" 1 $ScriptName $LogFile
...
if (Test-Path HKLM:\SOFTWARE\1E\NomadBranch)
{
    ...
    {
        Delete-OldVersions $TarPath $LogFile
    }
}

New-LogEntry "Finished" 1 $ScriptName $LogFile