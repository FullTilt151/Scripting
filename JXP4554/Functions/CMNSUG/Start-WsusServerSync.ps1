...
Begin {
    ...
    If ($EmailLog) {
        ...
        Function SendEmailStatus($From, $To, $Subject, $SmtpServer, $BodyAsHtml, $Body) {	
            ...
            If ($? -eq $False) {
                Write-Warning "$($Error[0].Exception.Message) | $($Error[0].Exception.GetBaseException().Message)"
            }
            $SmtpMessage.Dispose()
            rv SmtpClient
            rv SmtpMessage
            ...
            Else {	
                ...
                If ($TrialRun -eq $False) {	
                    ...
                    While ($SynchronizationProgress.Phase.ToString() -ne "NotProcessing") {	
                        If ($SynchronizationProgress.Phase.ToString() -eq "Categories") {$Object.Categories = $SynchronizationProgress.TotalItems}
                        If ($SynchronizationProgress.Phase.ToString() -eq "Updates") {$Object.Updates = $SynchronizationProgress.TotalItems}
                        If ($SynchronizationProgress.Phase.ToString() -eq "Approvals") {$Object.Approvals = $SynchronizationProgress.TotalItems}
