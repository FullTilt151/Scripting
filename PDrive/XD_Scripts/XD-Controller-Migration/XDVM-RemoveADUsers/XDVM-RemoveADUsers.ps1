...
$user_List = 'users.csv'
$users = import-csv $user_List -Header @("uid","ADGroup")
...
foreach ($row in $users) {
    ...
    Remove-ADGroupMember -Identity $row.ADGroup -Members $user -Confirm:$false
        ...
        if ($error.count -gt 0)   #Test if the addition of AD account to group was successful or not. Report the results. 
            ...
            $Worksheet.cells.Item($UIDCount + 1,2) = "Removal of AD User Failed"
            $Error.Clear()     #Clear the Error array.
            }
        else
            {
            $Worksheet.cells.Item($UIDCount + 1,2).Font.ColorIndex = 4  #4 is Green
            $Worksheet.cells.Item($UIDCount + 1,2) = "AD User removed successfully"
            }
}