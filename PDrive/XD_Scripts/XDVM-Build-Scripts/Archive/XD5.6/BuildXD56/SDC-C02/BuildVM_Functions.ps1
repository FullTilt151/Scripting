...
Get-VMHost -Name $ESX_Host | Get-Datastore | %{
      if($_.FreeSpaceMB -gt ($freeGBTgt * 1KB) -and $_.Name -like "*XEN_C02_PURE_VDI02_13" -and $_.Name -notlike "MGMT*" -and $_.Name -notlike "*storage-1" -and $_.Name -notlike "*250*" -and $_.FreeSpaceMB -gt $maxDsFree){
            $DataStoreName = $_.Name
            $maxDsFree = $_.FreeSpaceMb
      }
      if($_.FreeSpaceMB -gt ($freeGBTgt * 1KB) -and $_.Name -like "*XEN_C02_PURE*" -and $_.Name -notlike "MGMT*" -and $_.Name -notlike "*storage-1" -and $_.Name -notlike "*250*" -and $_.FreeSpaceMB -gt $maxDsFree){
            $DataStoreName = $_.Name
            $maxDsFree = $_.FreeSpaceMb
      }
}
