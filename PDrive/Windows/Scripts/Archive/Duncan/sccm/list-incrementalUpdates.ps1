...
gwmi sms_collection -computer $ServerName `
  ...
  if ($Coll.RefreshType -eq 6 -And $Coll.CollectionID -notlike "SMS*") {
        write-host "Incremental Updates enabled on: " $Coll.CollectionID "`t" $Coll.Name -ForegroundColor Yellow
        #$Coll.RefreshType = 2
        #$Coll.Put() | Out-Null
        $Count ++
      }
   }
Write-Host $Count "Collections were listed."
}
