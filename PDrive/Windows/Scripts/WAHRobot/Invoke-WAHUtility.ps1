...
function InstallWAHUtility {
    if ($admin) {
        ...
        if (get-item "C:\users\public\desktop\Humana W@H Robot.lnk" -force -ErrorAction SilentlyContinue) {
            Write-Output "Removing old W@H Robot desktop icon..."
            Remove-Item "C:\users\public\desktop\Humana W@H Robot.lnk" -force -ErrorAction SilentlyContinue
        }
        if (get-item "C:\users\public\desktop\Humana W@H Utility.lnk" -force -ErrorAction SilentlyContinue) {
            Write-Output "Removing old W@H Utility desktop icon..."
            Remove-Item "C:\users\public\desktop\Humana W@H Utility.lnk" -force -ErrorAction SilentlyContinue
        }
        if (get-item "C:\humscript\WAHUtility\Humana W@H Utility.lnk" -force -ErrorAction SilentlyContinue) {
            Write-Output "Removing old W@H shortcut in HUMSCRIPT..."
            Remove-Item "C:\humscript\WAHUtility\Humana W@H Utility.lnk" -force -ErrorAction SilentlyContinue
        }
        if (get-item "C:\humscript\WAHUtility\Humana W@H Utility - Email.lnk" -force -ErrorAction SilentlyContinue) {
            Write-Output "Removing old W@H shortcut in HUMSCRIPT..."
            Remove-Item "C:\humscript\WAHUtility\Humana W@H Utility - Email.lnk" -force -ErrorAction SilentlyContinue
        }
        if (get-item "C:\humscript\WAHUtility\Humana W@H Utility - PingPlotter.lnk" -force -ErrorAction SilentlyContinue) {
            Write-Output "Removing old W@H shortcut in HUMSCRIPT..."
            Remove-Item "C:\humscript\WAHUtility\Humana W@H Utility - PingPlotter.lnk" -force -ErrorAction SilentlyContinue
        }
        if (get-item "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Humana W@H Utility" -force -ErrorAction SilentlyContinue) {
            Write-Output "Removing old W@H shortcut in Start Menu..."
            Remove-Item "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Humana W@H Utility" -Recurse -force -ErrorAction SilentlyContinue
        }

        if (get-item C:\humscript\WAHRobot -ErrorAction SilentlyContinue) {
            Write-Output "Removing old script directory..."
            Remove-Item c:\humscript\WAHRobot -Recurse -Force -ErrorAction SilentlyContinue
        }
        ...
        if ([IntPtr]::Size -eq 8) {
            ...
            Copy-Item -Path .\Robot.ico -Destination C:\windows\SysNative -Recurse -Force -ErrorAction SilentlyContinue
        }
        ...
        if ([IntPtr]::Size -eq 4) {
            ...
            New-Item HKLM:\Software\Wow6432Node\Humana\WAHUtility -Force | Out-Null
            Set-ItemProperty HKLM:\Software\Wow6432Node\Humana\WAHUtility -Name 'Version' -Value $version -Force | Out-Null
            Set-ItemProperty HKLM:\Software\Wow6432Node\Humana\WAHUtility -Name 'Installed' -Value $date -Force | Out-Null
        }
    }
    Get-ChildItem C:\humscript\WAHUtility -include *.tmp -Hidden | foreach ($_) {remove-item $_.fullname}
}
...
function UninstallWAHUtility {
    if ($admin) {
        ...
        if ([IntPtr]::Size -eq 8) {
            ...
            Remove-Item 'C:\windows\SysNative\Robot.ico' -Force -ErrorAction SilentlyContinue
        }
        ...
        if ([IntPtr]::Size -eq 4) {
            ...
            Remove-Item HKLM:\SOFTWARE\Wow6432Node\humana\WAHUtility -Force -ErrorAction SilentlyContinue
        }
    }
}
...
if (!$InstallOnly -and !$Uninstall) {
    ...
    function ConnectionVerification {
        ...
        if ((($ips -like "133.17*").Count -gt 0 -or ($ips -like "10.94*").Count -gt 0) -and (Get-Process -Name arr_exe -ErrorAction SilentlyContinue)) {
            ...
            $Connection = "Aruba"
        } elseif (test-connection -count 1 LOUNASWPS01.rsc.humad.com -ErrorAction SilentlyContinue) {
    ...
    if ($Connection -eq "Array" -or $Connection -eq "Aruba" -or $Connection -eq "OnNetwork") {
        ...
        Test-Connection -Count 1 LOUNASWPS01.rsc.humad.com -Verbose -ErrorAction SilentlyContinue | Format-Table Address, Ipv4Address, ResponseTime -AutoSize

        Write-Output "`n##### Internal Ping test via IP #####"
        Test-Connection -Count 1 193.111.20.111 -Verbose -ErrorAction SilentlyContinue | Format-Table Address, Ipv4Address, ResponseTime -AutoSize
    }
    ...
    function Big5Compliance {
        ...
        $serv = (sc.exe query)
        ($serv[$serv.indexof(($serv | Select-String DGScan))]) + " " + (($serv[$serv.indexof(($serv | Select-String DGScan))+3]))
        ($serv[$serv.indexof(($serv | Select-String DGServic))]) + " " + (($serv[$serv.indexof(($serv | Select-String DGService))+3]))
        ...
        Write-Output "McAfee Shield: $((Get-Service -Name McShield -ErrorAction SilentlyContinue).Status)"
        Write-Output "McAfee Task Manager: $((Get-Service -Name McTaskManager -ErrorAction SilentlyContinue).Status)"
        Write-Output "McAfee DAT Date: $((Get-ItemProperty HKLM:\SOFTWARE\Wow6432Node\McAfee\AVEngine -Name AVDatDate -ErrorAction SilentlyContinue).AvDatDate)"
