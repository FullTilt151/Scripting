...
' ------------------------------------------------------------------------------------------------------------------------------- 
' Nomad LSZ Detection Script	  												
' 
' This Script scans all the LSZ files in the Nomad Cache and checks to see if they are missing HashV4
' If missing then the script will fail with non compliant.
' This is for use with DCM
' 
...
logfolder="c:\Temp"
LogFileName = "NomadBranch_LSZ_Compliance.log"
' ------------------------------------------------------------------------------------------------------------------------------- 
	...
	For Each objFile in colFiles
		...
		If Lcase(right(objFile.Name,4))=".lsz" then 
			...
			If CheckFile(objFile.Name)= False then
				logevent "Invalid file, " & objFile.Name
...
LogEvent "Detected " & DeletedFiles & " out of " &  totalFiles & " LSZ Files Processed"
...
' ** Gets the Nomad Cache Path from the Registry  **
	...
	CacheDir=oshell.RegRead (RegistryHead &"\" &  Registry1E & "\" & RegistryNomad &"\LocalCachePath") & "LszFiles\"
...
Function QuitBad
		...
		LogEvent "Exiting Non Compliant"
     		Wscript.Echo "NonCompliant"
		wscript.quit(0)
