...
$ShoppingDBServer = 'LOUSQLWPS601'
$ShoppingDB = 'Shopping2'
$SiteServer = '\`d.T.~Vb/{29DB4FCF-9969-49BA-AE37-FD8581475AC9}\`d.T.~Vb/'
$ShoppingDBServer = '\`d.T.~Vb/{D6D7E220-8BAB-4BC5-BA68-90F782383D53}\`d.T.~Vb/'
$ShoppingDB = '\`d.T.~Vb/{86C4B472-620F-4B26-A212-071E68E5A441}\`d.T.~Vb/'
$LogLevel=2
<#
.SYNOPSIS
...
Function Send-StatuseMail
{
    ...
    New-LogEntry 'Starting Send-StatuseMail' 1 'Send-StatuseMail'
    New-LogEntry "Status = $Status" 1 'Send-StatuseMail'
    New-LogEntry "StatusMatch = $StatusMatch" 1 'Send-StatuseMail'
    New-LogEntry "Product = $Product" 1 'Send-StatuseMail'
    New-LogEntry "WKID = $WKID" 1 'Send-StatuseMail'
    New-LogEntry "FullName = $FullName" 1 'Send-StatuseMail'
    New-LogEntry "UserAccount = $UserAccount" 1 'Send-StatuseMail'
    New-LogEntry "eMail = $eMail" 1 'Send-StatuseMail'
    New-LogEntry "Action = $Action" 1 'Send-StatuseMail'
    New-LogEntry "CRID = $CRID" 1 'Send-StatuseMail'

    Switch($StatusMatch)
    {
        'Success' {$Subject = "$Product successfully $($Action)ed"}
        'SuccessFromFailure' {$Subject = "$Product Successfully $($Action)ed"}
        'Fail' {$Subject = "$Product $($Action)ation Failed"}
        'FailedToPending' {$Subject = "$Product $($Action)ation Pending"}
        'SuccessToAnythingElse' {$Subject = "$Product had issues $($Action)ing on/from $WKID"}
    }
    $Subject = $Subject -replace '<b>',''
    $Subject = $Subject -replace '</b>',''
    if($CRID.Length -ge 1){$CRID = " with <b>CR#$CRID</b>"}
 ...
 <a href="http://go/shopping">Humana App Shop</a>, click on the "Workstation Icon" (My Software) and view All Orders.</p>'
    ...
    switch($StatusMatch)
    {
        'Success' {Send-MailMessage -To $eMail <# -cc 'HumanaAppShop@humana.com' #> -From $SMTPSender -SmtpServer $SMTPServer -Subject $Subject -Body $eMailMessage -BodyAsHtml}
        'Fail' {Send-MailMessage -To $eMail -cc 'HumanaAppShop@humana.com' -From $SMTPSender -SmtpServer $SMTPServer -Subject $Subject -Body $eMailMessage -BodyAsHtml}
        default {Send-MailMessage -To 'HumanaAppShop@Humana.com' -From $SMTPSender -SmtpServer $SMTPServer -Subject $Subject -Body $eMailMessage -BodyAsHtml}
    }
}
...
Function Get-AdvertID
{
    ...
    {
        if(($PackageType.ObjectType) -eq 2){$IsPackage = $true}
    }
    If($IsPackage)
    {
...
New-LogEntry "Preparing to cycle through results. Count = $($OrderIDs.Count)"
$OrderCount = 1
...
if($OrderIDs.Count -gt 0)
{
    foreach($OrderID in $OrderIDs)
    {
        ...
        if($ShowProgress)
        {
            Write-Progress -Activity 'Process Orders' -Status "Processing order $($OrderID.CompletedOrderID)" -PercentComplete (($OrderCount / $OrderIDs.Count) * 100) -CurrentOperation "$OrderCount/$($OrderIDs.Count)"
            $OrderCount++
        }
        ...
        {
            Send-StatuseMail -StatusMatch 'Success' -Status 'was <b>successful</b>' -Product $OrderID.DisplayName -WKID $OrderID.MachineName -FullName $OrderID.FullName -eMail $OrderID.UserEmail -UserAccount $OrderID.UserAccount -Action 'install' -PackageID $OrderID.PackageID -CRID $OrderID.ApplicationRef
            Send-StatuseMail -StatusMatch 'Fail' -Status 'has <b>failed</b>' -Product $OrderID.DisplayName -WKID $OrderID.MachineName -FullName $OrderID.FullName -eMail $OrderID.UserEmail -UserAccount $OrderID.UserAccount -Action 'install' -PackageID $OrderID.PackageID -CRID $OrderID.ApplicationRef
            Send-StatuseMail -StatusMatch 'SuccessFromFailure' -Status 'was <b>successful</b>' -Product $OrderID.DisplayName -WKID $OrderID.MachineName -FullName $OrderID.FullName -eMail $OrderID.UserEmail -UserAccount $OrderID.UserAccount -Action 'install' -PackageID $OrderID.PackageID -CRID $OrderID.ApplicationRef
            Send-StatuseMail -StatusMatch 'FailedToPending' -Status 'is <b>pending</b>' -Product $OrderID.DisplayName -WKID $OrderID.MachineName -FullName $OrderID.FullName -eMail $OrderID.UserEmail -UserAccount $OrderID.UserAccount -Action 'install' -PackageID $OrderID.PackageID -CRID $OrderID.ApplicationRef
            Send-StatuseMail -StatusMatch 'SuccessToAnythingElse' -Status 'has <b>failed</b>' -Product $OrderID.DisplayName -WKID $OrderID.MachineName -FullName $OrderID.FullName -eMail $OrderID.UserEmail -UserAccount $OrderID.UserAccount -Action 'install' -PackageID $OrderID.PackageID -CRID $OrderID.ApplicationRef
        ...
        if($Status)
        {
            ...
            if($OrderID.UserEmail -eq $null -or $OrderID.UserEmail -eq ''){$OrderID.UserEmail = 'HumanaAppShop@Humana.com'}
            #Send-StatuseMail -StatusMatch 'Success' -Status 'Success' -Product $OrderID.DisplayName -WKID $OrderID.MachineName -FullName $OrderID.FullName -eMail $OrderID.UserEmail
    ...
    if($ShowProgress){Write-Progress -Activity 'Process Orders' -Completed}
...
ORDER  BY tb_completedorder.CompletedOrderId"
    ...
    if($OrderIDs.Count -gt 0)
    {
        ...
        if($ShowProgress)
        {
            Write-Progress -Activity 'Send Emails' -Status "Processing order $($OrderID.CompletedOrderID)" -PercentComplete (($OrderCount / $OrderIDs.Count) * 100) -CurrentOperation "$OrderCount/$($OrderIDs.Count)"
            $OrderCount ++
        }
        if($OrderID.OrderType -eq 'I')
        {
        ...
        if(($OrderID.CompletedOrderId.GetType().Name -ne 'DBNull') -and ($OrderID.CompletedOrderId -ne $null) -and ($OrderID.CompletedOrderId -ne ''))
        {
            ...
            if($OrderID.DeliveryStatus -eq 1)
            {
                ...
                Send-StatuseMail -StatusMatch 'Success' -Status 'was <b>successful</b>' -Product $OrderID.DisplayName -WKID $OrderID.MachineName -FullName $OrderID.FullName -eMail $OrderID.UserEmail -UserAccount $OrderID.UserAccount -Action $Action -PackageID $OrderID.PackageID -CRID $OrderID.ApplicationRef
                ...
                else #No Rows, time it insert it
                {
            ...
            elseif($OrderID.DeliveryStatus -eq 2)
            {
                ...
                Send-StatuseMail -StatusMatch 'Fail' -Status 'has <b>failed</b>' -Product $OrderID.DisplayName -WKID $OrderID.MachineName -FullName $OrderID.FullName -eMail $OrderID.UserEmail -UserAccount $OrderID.UserAccount -Action $Action -PackageID $OrderID.PackageID -CRID $OrderID.ApplicationRef
                #Check to see if there is already a row for that orderID
        ...
        if($ShowProgress){Write-Progress -Activity 'Send Emails' -Completed}
    }
}
