...
SELECT		DISTINCT sys.Netbios_Name0, 
			(select top 1 LastStatusMessageIDName 
				from v_ClientAdvertisementStatus 
				where ResourceID = CAS.ResourceID
				order by LastStatusTime), 
			(select top 1 LastStateName
				from v_ClientAdvertisementStatus
				where ResourceID = CAS.ResourceID
				order by LastStatusTime), 
			(select top 1 DATEADD(HOUR,-5, LastStatusTime)
				from v_ClientAdvertisementStatus
				where ResourceID = CAS.ResourceID
				order by LastStatusTime) AS [LastStatusTime]
...
SELECT    DISTINCT CAS.ResourceID, COUNT(LastStateName) as [count]
FROM         v_ClientAdvertisementStatus CAS
join	v_R_System sys on sys.ResourceID = CAS.ResourceID
WHERE     (
				AdvertisementID = 'CAS23183' OR
